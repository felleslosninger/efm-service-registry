...
public class ServiceRecordFactoryTest {
    ...
    @Test(expected = SecurityLevelNotFoundException.class)
    public void createArkivmeldingServiceRecordsWithSecurityLevel4_IdentifierHasSvarUtRegistrationOnDifferentLevel_ShouldThrowDedicatedException()
        ...
        factory.createArkivmeldingServiceRecords(ORGNR_FIKS, fakeOauthAuthentication(Lists.newArrayList(DPF_SCOPE)), 4);
    }

    @Test
    public void createArkivmeldingServiceRecordsWithSecurityLevelDifferentFrom4_IdentifierHasSvarUtRegistrationOnAnotherLevel_ShouldReturnDpvServiceRecord()
            throws SecurityLevelNotFoundException, CertificateNotFoundException, SvarUtClientException {
        when(svarUtService.hasSvarUtAdressering(eq(ORGNR_FIKS), any())).thenReturn(Optional.empty());
        List<ServiceRecord> result = factory.createArkivmeldingServiceRecords(ORGNR_FIKS, fakeOauthAuthentication(Lists.newArrayList(DPF_SCOPE)), 3);

        assertTrue(result.stream().anyMatch(record->record.getService().getIdentifier().equals(ServiceIdentifier.DPV)));
    }
    ...
    public void createDigitalpostServiceRecords_ForcePrintMessageToRecipientNotInPopulationRegistry_ShouldThrowDedicatedException()
        ...
        factory.createDigitalpostServiceRecords(PERSONNUMMER, fakeOauthAuthentication(Lists.newArrayList(DPI_SCOPE)), "991825827");
        unsetFakeOauthAuthentication();
    }
