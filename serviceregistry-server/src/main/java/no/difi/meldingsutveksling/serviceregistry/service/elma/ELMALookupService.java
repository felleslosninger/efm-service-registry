...
//                    TODO: verify that the list of process identifier only has one object. No documentation on this behavior has been found in Peppol docs. Potential bug alert!
    ...
    public List<ServiceMetadata> lookup(Iso6523 identifier, Set<String> documentIdentifiers) {
        ...
        try {
            Set<String> registeredIdentifiers = lookupClient.getDocumentIdentifiers(ParticipantIdentifier.of(identifier.toString()))
            ...
            for (String id : documentIdentifiers) {
                ...
                if (registeredIdentifiers.contains(id)) {
                    ...
                    log.debug("Skipping ELMA lookup for non-registered document type: identifier={}, documentTypeIdentifier={}", identifier, id);
                }
            }
            ...
            log.debug("Failed ELMA lookup for identifier={}, documentTypeIdentifier={}", identifier, logId, e);
        }
        catch (NullPointerException e) {
            log.debug("Organization {} is not in ELMA", identifier.toString(), e);
        }
        return metadataList;
    }
