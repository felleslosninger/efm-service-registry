...
public class NhnService {
    ...
    @Cacheable(CacheConfig.NHN_CACHE)
    public HealthAddressRegistryDetails getARDetails(LookupParameters param) {
            ...
            return restClient.get()
                    ...
                    .onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
                        throw new ServiceRegistryException(
                                "Internal server fail while getting AR details for id " + param.getIdentifier());
                    }).toEntity(HealthAddressRegistryDetails.class).getBody();
        } catch (ResourceAccessException e) {
            log.warn("Healthcare service is down");
            throw e;
