...
class ServiceRecordFactory(
    ...
    fun createDpfServiceRecord(identifier: Iso6523, process: Process, securityLevel: Int): ServiceRecord {
        val serviceRecord = ServiceRecord(
            ...
            process,
            properties.fiks.svarut.serviceRecordUrl.toString()
        )
    ...
    fun createDigitalDpvServiceRecord(identifier: PersonIdentifier, process: Process): ServiceRecord {
        val dpvServiceRecord =
            ServiceRecord(ServiceIdentifier.DPV, identifier.identifier, process, properties.dpv.endpointURL.toString())
    ...
    fun createPrintServiceRecord(
        ...
        val fregGatewayEntity = kontaktInfoService.getFregAdress(LookupParameters.lookup(identifier))
            .orElseThrow { FregGatewayException("Receiver found in KRR on behalf of '$clientIdentifier', but not in FREG.") }
    ...
    fun createDpvServiceRecord(identifier: Iso6523, process: Process): ServiceRecord {
        val dpvServiceRecord = ServiceRecord(
            ServiceIdentifier.DPV,
            identifier.orgIdentifier(),
            process,
            properties.dpv.endpointURL.toString()
        )
